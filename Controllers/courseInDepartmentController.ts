...
const getCoursesInDepartments = (req: any, res: any) => {
    ...
    let departId = req.query.department ? req.query.department : "";
    console.log(departId,"***************##################")
      CourseInDepartmentModel.find({department:departId}).populate({path:'course', populate:{path:"lecturers"}}).exec((err,data)=>{
        if(!err){
                   console.log(data)
...
const getCourseInDepartmentById = (req: any, res: any) => {
  ...
  CourseInDepartmentModel.findById(id,).populate({path:'course', populate:{path:"lecturers"}}).exec((err,data)=>{
    if(!err){
               console.log(data)
          res.status(response.OK_200);
          res.json({
